...
- `language`: optional `zh-CN` or `en`; defaults to `zh-CN` when omitted.
...
The builder copies images into the output package and emits relative paths only. `handoff.md` uses Simplified Chinese by default; set `"language": "en"` for English.
